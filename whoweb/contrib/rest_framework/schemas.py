            ...
            for method in paths[route]:
                ...
                if operation_id in ids:
                    warnings.warn(
                        ...
                        "\tAn operationId has to be unique accros your schema. Your schema may not work in other tools.".format(
...
class FutureAutoSchema(ViewInspector):
...
class VerboseAutoSchema(FutureAutoSchema):
    ...
    base_methods = ["retrieve", "create", "update", "partial_update", "destroy"]
    ...
    def get_summary(self, path, method):
        ...
        view_name = getattr(self.view, "name", None)
        if view_name and action_name not in self.base_methods:
            return view_name
        if is_list_view(path, method, self.view):
            action = "list"
        elif action_name and action_name not in self.base_methods:
            action = self.method_mapping[method.lower()] + action_name
        else:
            action = self._to_camel_case(self.method_mapping[method.lower()])

        name = self.get_operation_id_base(path, method, action)
        return camel_case_to_spaces(action + name).title()
    ...
    def get_request_component(self, path, method):
    ...
    def get_response_component(self, path, method):
    ...
    def get_components(self, path, method):
        if method.lower() == "delete":
            return {}
        request: dict = self.get_request_component(path, method)
        response: dict = self.get_response_component(path, method)
