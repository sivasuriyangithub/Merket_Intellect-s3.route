...
class CampaignList(ColdemailBaseModel):
    ...
    def publish(self, apply_tasks=True, on_complete=None):
        ...
        export = self.data_source.export

        if not export and not self.data_source.query:
            raise AttributeError("List data source has neither query nor export.")
        elif not export:
            export = self.data_source.convert_query_to_export()
    ...
    def api_upload(self, task_context=None):
        ...
        created = self.api_class.create_by_url(
            url=self.data_source.export.get_named_fetch_url()
        )
    ...
    def get_recipient_emails(self):
        return (
            row.email for row in self.data_source.export.get_profiles() if row.email
        )
